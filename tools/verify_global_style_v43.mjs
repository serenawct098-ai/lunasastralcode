...
const requiredFiles = [
  ...
  'governance/script_theme_arc_v43.json',
...
try {
  ...
  themeMap = JSON.parse(fs.readFileSync(path.join(root, 'governance/script_theme_arc_v43.json'), 'utf8'));
} catch (error) {
...
const checks = [
  [
    'Master Playbook 使用 v4.3 小說式描寫規則庫',
    playbook.includes('小說式描寫文風（v4.3｜唯一全局文風規範，奇門專用）'),
  ],
  [
    '四大型式均採小說式描寫，且奇門內容仍盤象先行',
    playbook.includes('型式一、三、四、五都使用**玄學小說式描寫**') &&
      playbook.includes('盤象先行，不可調換') &&
      writing.all_forms?.length === 5 && writing.qimen_gate?.includes('盤象先行'),
  ],
  [
  ...
  [
    '規則映射與主題地圖均為 v4.3，且包含 23 篇與 9/28 配對',
    writing.version === '4.3' && themeMap?.version === '4.3' &&
  ...
  [
    '腳本索引已同步為 v4.3',
    index.includes('小說式描寫文風 v4.3') && !index.includes('使用者文章風格 v3.4') &&
